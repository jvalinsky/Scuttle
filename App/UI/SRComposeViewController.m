...
- (void)setReplyToKey:(NSString *)replyToKey {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (replyToKey.length > 0) {
            self.replyLabel.stringValue = [NSString stringWithFormat:@"Replying to... %@", [replyToKey substringToIndex:MIN(10, replyToKey.length)]];
        }
    });
}
...
- (void)setupUI {
    ...
    self.replyLabel = [NSTextField labelWithString:@"Replying to..."];
    ...
    self.charCountLabel = [NSTextField labelWithString:@"0 / 1000"];
...
- (void)publishAction:(id)sender {
    ...
    if (self.view.window.contentView) {
        [SRNotificationBannerView showInView:self.view.window.contentView message:@"Message published successfully!" type:SRNotificationTypeSuccess];
    }
...
- (void)textDidChange:(NSNotification *)notification {
    ...
    self.charCountLabel.stringValue = [NSString stringWithFormat:@"%ld / 1000", (long)count];
    if (count > 1000) {
